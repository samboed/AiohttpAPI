...
from src.api.decorators import owner_required
...
class BaseItemView(web.View):
    ...
    async def put(self, item):
        ...
        add_item(self.request.session, item)
        return web.json_response(item.dict)
    ...
    async def delete(self, item):
        delete_item(self.request.session, item)
...
class BaseGroupView(web.View):
    ...
    async def get(self):
        item_group = get_group(self.request.session, self.model)
...
class BaseTokenItemView(web.View):
    ...
    def _get_item_by_token(self):
        user_id = get_user_id_from_token(self.request)
        item = get_item(self.request.session, self.model, user_id)
        return item

    @login_required
    async def get(self):
        item = self._get_item_by_token()
        return web.json_response(item.dict)
    ...
    async def delete(self):
        user = self._get_item_by_token()
        delete_item(self.request.session, user)
